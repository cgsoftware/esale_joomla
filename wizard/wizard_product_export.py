    ...
    for website in self.pool.get('esale_joomla.web').browse(cr, uid, web_ids):
        ...
        for categ in website.category_ids:
            ...
            for product in self.pool.get('product.product').browse(cr, uid, export_products, context=context):
                ...
                #Esportazione attributi modulo Electronic_product
                p = {}
                z=0
                for attr in product.attribute_ids:
                    attr.value = attr.value.replace('<', '|min|')
                    attr.value = attr.value.replace('>', '|mag|')
                    attr.value = attr.value.replace('&', '|uppersend|')
                    p[attr.name] = attr.value or ''
                ...
                webproduct = {
                    ...
                    'product_weight': product.weight_net,
                    'product_length': product.product_lenght,
                    'product_width': product.product_width,
                    'product_height': product.product_height,
                    'lenght_uom': product.lenght_uom, 
                    ...
                    'code':str(product.default_code or ''), 
                    'description':product.description or '',
                    'prod_update_count': str(prod_update_count) or '9',
                    'slideshow':str(product.slideshow) or '',                    
                    #'en_product':en_product, 
                    #'short_description_it':it_product.description_sale or '', 
                    #'short_description_en':en_product.description_sale or '', 
                }
                
                
                
                webproduct['img_small'] = 0
                if product.picture  :
                    webproduct['img_small'] = 1
                    webproduct['picture_thumb'] = product.picture
    
                
                #Esportazione immagini big
                #import pdb; pdb.set_trace()
                ...
                #luca webproduct['description'] = str((product.description_sale or '') + (len(taxes_name) and ("\n(" + (', '.join(taxes_name)) + ')') or ''))
                ...
                try:
                    ...
                    if webproduct['haspic'] or webproduct['img_small'] :
                        prod_img_count += webproduct['haspic'] + webproduct['img_small'] 
